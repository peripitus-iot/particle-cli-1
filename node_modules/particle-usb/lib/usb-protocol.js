...
exports.recvRequest = recvRequest;
exports.resetRequest = resetRequest;
exports.parseReply = parseReply;
exports.encodeReply = encodeReply;
exports.MAX_PAYLOAD_SIZE = exports.MAX_REQUEST_TYPE = exports.MAX_REQUEST_ID = exports.MIN_WLENGTH = exports.PARTICLE_BREQUEST = exports.BmRequestType = exports.Status = exports.FieldFlag = exports.ServiceType = void 0;
...
const BmRequestType = {
  HOST_TO_DEVICE: 0x40,
  // 01000000b (direction: host-to-device; type: vendor; recipient: device)
...
exports.BmRequestType = BmRequestType;
const PARTICLE_BREQUEST = 0x50; // ASCII code of the character 'P'
// Minimum length of the data stage for high-speed USB devices

exports.PARTICLE_BREQUEST = PARTICLE_BREQUEST;
const MIN_WLENGTH = 64; // Misc. constraints defined by the protocol and the USB specification

exports.MIN_WLENGTH = MIN_WLENGTH;
const MAX_REQUEST_ID = 0xffff;
exports.MAX_REQUEST_ID = MAX_REQUEST_ID;
const MAX_REQUEST_TYPE = 0xffff;
exports.MAX_REQUEST_TYPE = MAX_REQUEST_TYPE;
const MAX_PAYLOAD_SIZE = 0xffff; // Returns the setup packet fields for the INIT service request
...
function resetRequest(reqId = 0) {
  return {
    ...
    wIndex: reqId,
    // Request ID (can be set to 0 to reset all requests)
    wValue: 0,
    // Not used
    wLength: MIN_WLENGTH
  };
...
function parseReply(data) {
  try {
    const rep = {};
    let offs = 0; // Field flags (4 bytes)

    rep.flags = data.readUInt32LE(offs);
    offs += 4; // Status code (2 bytes)

    if (!(rep.flags & FieldFlag.STATUS)) {
      throw new _error.ProtocolError('Service reply is missing mandatory status field');
    }

    rep.status = data.readUInt16LE(offs);
    offs += 2; // Request ID (2 bytes, optional)
    ...
    if (!(err instanceof _error.ProtocolError)) {
      throw new _error.ProtocolError(err, 'Unable to parse service reply');
    }
...
//# sourceMappingURL=usb-protocol.js.map