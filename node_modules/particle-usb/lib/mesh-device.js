...
exports.MeshDevice = exports.DiagnosticType = void 0;
...
const DiagnosticType = (0, _protobufUtil.fromProtobufEnum)(_protocol.default.mesh.DiagnosticType, {
...
function transformNetworkDiagnosticInfo(info) {
  const result = {};
  const leaderRlocs = new _set.default();
  const gatewayRlocs = new _set.default();
...
const MeshDevice = base => class extends base {
   ...
   */
  async meshAuth(pwd) {
    return this.sendRequest(_request.Request.MESH_AUTH, {
      password: pwd
    });
  }
  /**
  ...
  async createMeshNetwork(network) {
    ...
    return this._runInListeningMode(async () => {
      const r = await this.sendRequest(_request.Request.MESH_CREATE_NETWORK, {
        name: network.name,
  ...
  async leaveMeshNetwork() {
    return this._runInListeningMode(() => {
      return this.sendRequest(_request.Request.MESH_LEAVE_NETWORK);
    });
  }
  /**
  ...
  async getMeshNetworkInfo() {
    const r = await this.sendRequest(_request.Request.MESH_GET_NETWORK_INFO, null, {
      dontThrow: true
    });

    if (r.result === _result.Result.NOT_FOUND) {
      return null; // The device is not a member of a network
    }

    if (r.result !== _result.Result.OK) {
      throw new _error.RequestError(r.result);
    }
  ...
  async startCommissioner(timeout) {
    return this.sendRequest(_request.Request.MESH_START_COMMISSIONER, {
      timeout: timeout
    });
  }
  /**
  ...
  async stopCommissioner() {
    return this.sendRequest(_request.Request.MESH_STOP_COMMISSIONER);
  }
  /**
   * Join the network.
   *
  ...
  async joinMeshNetwork(commDev) {
    return this._runInListeningMode(async () => {
      ...
      let r = await commDev.sendRequest(_request.Request.MESH_GET_NETWORK_INFO);
      const network = r.network;
      r = await this.sendRequest(_request.Request.MESH_PREPARE_JOINER, {
        network: network
      });
      const eui64 = r.eui64;
      const joinPwd = r.password;
      await commDev.sendRequest(_request.Request.MESH_ADD_JOINER, {
        eui64: eui64,
        password: joinPwd
      });
      await this.sendRequest(_request.Request.MESH_JOIN_NETWORK);
    });
  }
  /**
  ...
  async scanMeshNetworks() {
    const r = await this.sendRequest(_request.Request.MESH_SCAN_NETWORKS);
  ...
  async setSetupDone(done) {
    ...
    return this.sendRequest(_request.Request.SET_DEVICE_SETUP_DONE, {
      done: done
    });
  }
  /**
  ...
  async _runInListeningMode(fn) {
    ...
    if (mode === _device.DeviceMode.LISTENING) {
      return fn();
    }
  ...
  async getMeshNetworkDiagnosticInfo(opts = {
    ...
    if (opts.queryChildren) {
      flags |= _protocol.default.mesh.GetNetworkDiagnosticsRequest.Flags['QUERY_CHILDREN'];
    }

    if (opts.resolveDeviceId) {
      flags |= _protocol.default.mesh.GetNetworkDiagnosticsRequest.Flags['RESOLVE_DEVICE_ID'];
    }

    const info = await this.sendRequest(_request.Request.MESH_GET_NETWORK_DIAGNOSTICS, {
      flags: flags,
...
exports.MeshDevice = MeshDevice;
//# sourceMappingURL=mesh-device.js.map