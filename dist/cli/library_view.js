...
const {
  CLILibraryInstallCommandSite
} = require('./library_install');
...
class CLILibraryViewCommandSite extends CLILibraryInstallCommandSite {
  ...
  view() {
    ...
    if (readme) {
      this.showFile('No readme files found for the library.', [path.join(targetDir, 'README.md'), path.join(targetDir, 'README.txt')]);
    }

    if (source) {
      this.showFile('No source files found for the library.', [path.join(targetDir, 'src', `${name}.cpp`)]);
    }

    if (header) {
      this.showFile('No header files found for the library.', [path.join(targetDir, 'src', `${name}.h`)]);
    }
    ...
    if (json) {
      ...
      return console.log(this.createJSONResult());
    }
  ...
  showFile(missing, filenames) {
    ...
    for (let filename of filenames) {
      ...
      if (content !== undefined) {
        if (json) {
          console.log(this.createJSONResult(content));
        } else {
          console.log(content);
        }
    ...
    if (!shown) {
      if (json) {
        console.log(this.createJSONResult());
      } else {
        console.log(missing);
      }
    }
...
module.exports.command = (apiJS, argv) => {
  ...
  return site.run(cmd).then(() => site.view()).catch(error => {
