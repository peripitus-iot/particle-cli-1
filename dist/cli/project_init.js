...
const {
  ProjectInitCommand,
  ProjectInitCommandSite,
  Projects
} = require('../cmd'); // todo - this is pulled from validating_editor in particle-dev-libraries. Please refactor/DRY.
...
function fieldValidator(fieldName) {
  const validator = value => {
    ...
    return result && result.valid ? '' : validationMessage(result, fieldName);
  };
...
class CLIProjectInitCommandSite extends ProjectInitCommandSite {
  ...
  async dialog() {
    ...
    const promptForLocation = (commonLocation, currentLocation) => {
      return this._dir || this.prompt('Would you like to create your project in the default project directory? [Y/n]', 'Y', yesNoValidator()).then(result => {
        if (isYes(result)) {
          return commonLocation;
        }
      }).then(location => {
        return location || this.prompt(`Would you like to create your project in ${chalk.bold(currentLocation)}? Type “n” to cancel. [Y/n]`, 'Y', yesNoValidator()).then(result => {
