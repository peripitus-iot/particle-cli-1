...
describe('Keys Command-Line Interface', () => {
  ...
  describe('Top-Level `keys` Namespace', () => {
    ...
    it('Includes help', () => {
      ...
      commandProcessor.showHelp(helpText => {
        expect(helpText).to.equal(['Manage your device\'s key pair and server public key', 'Usage: particle keys <command>', 'Help:  particle help keys <command>', '', 'Commands:', '  new       Generate a new set of keys for your device', '  load      Load a key saved in a file onto your device', '  save      Save a key from your device to a file', '  send      Tell a server which key you\'d like to use by sending your public key in PEM format', '  doctor    Creates and assigns a new key to your device, and uploads it to the cloud', '  server    Switch server public keys.', '  address   Read server configured in device server public key', '  protocol  Retrieve or change transport protocol the device uses to communicate with the cloud', ''].join(os.EOL));
      });
    });
  });
  describe('`keys new` Namespace', () => {
    ...
    it('Includes help', () => {
      ...
      commandProcessor.showHelp(helpText => {
        expect(helpText).to.equal(['Generate a new set of keys for your device', 'Usage: particle keys new [options] [filename]', '', 'Options:', '  --protocol  Communication protocol for the device using the key. tcp or udp  [string]', ''].join(os.EOL));
      });
    });
  });
  describe('`keys claim` Namespace', () => {
    ...
    it('Includes help', () => {
      ...
      commandProcessor.showHelp(helpText => {
        expect(helpText).to.equal(['Load a key saved in a file onto your device', 'Usage: particle keys load [options] <filename>', ''].join(os.EOL));
      });
    });
  });
  describe('`keys save` Namespace', () => {
    ...
    it('Includes help', () => {
      ...
      commandProcessor.showHelp(helpText => {
        expect(helpText).to.equal(['Save a key from your device to a file', 'Usage: particle keys save [options] <filename>', '', 'Options:', '  --force  Force overwriting of <filename> if it exists  [boolean] [default: false]', ''].join(os.EOL));
      });
    });
  });
  describe('`keys send` Namespace', () => {
    ...
    it('Includes help', () => {
      ...
      commandProcessor.showHelp(helpText => {
        expect(helpText).to.equal(['Tell a server which key you\'d like to use by sending your public key in PEM format', 'Usage: particle keys send [options] <deviceID> <filename>', '', 'Options:', '  --product_id  The product ID to use when provisioning a new device  [number]', ''].join(os.EOL));
      });
    });
  });
  describe('`keys doctor` Namespace', () => {
    ...
    it('Includes help', () => {
      ...
      commandProcessor.showHelp(helpText => {
        expect(helpText).to.equal(['Creates and assigns a new key to your device, and uploads it to the cloud', 'Usage: particle keys doctor [options] <deviceID>', '', 'Options:', '  --protocol  Communication protocol for the device using the key. tcp or udp  [string]', ''].join(os.EOL));
      });
    });
  });
  describe('`keys server` Namespace', () => {
    ...
    it('Parses optional params', () => {
      ...
      expect(argv.params).to.eql({
        filename: '/path/to/my-key.pem',
        outputFilename: '/path/to/output.pem'
      });
      expect(argv.protocol).to.equal(undefined);
      expect(argv.host).to.equal(undefined);
      expect(argv.port).to.equal(undefined);
    ...
    it('Includes help', () => {
      ...
      commandProcessor.showHelp(helpText => {
        expect(helpText).to.equal(['Switch server public keys.', 'Usage: particle keys server [options] [filename] [outputFilename]', '', 'Options:', '  --protocol    Communication protocol for the device using the key. tcp or udp  [string]', '  --host        Hostname or IP address of the server to add to the key  [string]', '  --port        Port number of the server to add to the key  [number]', '  --deviceType  Generate key file for the provided device type  [string]', '', 'Defaults to the Particle public cloud or you can provide another key in DER format and the server hostname or IP and port', ''].join(os.EOL));
      });
    });
  });
  describe('`keys address` Namespace', () => {
    ...
    it('Includes help', () => {
      ...
      commandProcessor.showHelp(helpText => {
        expect(helpText).to.equal(['Read server configured in device server public key', 'Usage: particle keys address [options]', '', 'Options:', '  --protocol  Communication protocol for the device using the key. tcp or udp  [string]', ''].join(os.EOL));
      });
    });
  });
  describe('`keys protocol` Namespace', () => {
    ...
    it('Includes help', () => {
      ...
      commandProcessor.showHelp(helpText => {
        expect(helpText).to.equal(['Retrieve or change transport protocol the device uses to communicate with the cloud', 'Usage: particle keys protocol [options]', '', 'Options:', '  --protocol  Communication protocol for the device using the key. tcp or udp  [string]', ''].join(os.EOL));
      });
    });
  });
});