...
describe('Publish Command-Line Interface', () => {
  ...
  describe('Top-Level `publish` Namespace', () => {
    ...
    it('Throws when option flag is malformed', () => {
      expect(() => commandProcessor.parse(root, ['publish', 'my-event', '--product'])).to.throw('Not enough arguments following: product');
    });
    it('Includes help with examples', () => {
      ...
      commandProcessor.showHelp(helpText => {
        expect(helpText).to.equal(['Publish an event to the cloud', 'Usage: particle publish [options] <event> [data]', '', 'Options:', '  --private  Publish to the private stream  [boolean] [default: true]', '  --public   Publish to the public stream  [boolean]', '  --product  Publish to the given Product ID or Slug\'s stream  [string]', '', 'Examples:', '  particle publish temp 25.0                  Publish a temp event to your private event stream', '  particle publish temp 25.0 --product 12345  Publish a temp event to your product 12345\'s event stream', ''].join(os.EOL));
      });
    });
  });
});