...
class CLILibraryListCommandSite extends LibraryListCommandSite {
  constructor(argv, apiClient) {
    ...
    this._page = this.argv.page || 1; // todo - since the text can be used by any app, this could be pushed down to the command layer so it's shared
  ...
  notifyFetchLists(promise) {
    ...
    return (json ? promise : spin(promise, msg)).then(results => {
      const sections = this.sectionNames();
      ...
      if (json) {
        ...
        return console.log(this.createJSONResult(page, data));
      }
...
module.exports.command = (apiJS, argv) => {
  ...
  function runPage() {
    return site.run(cmd).then(results => nextPage(results)).catch(error => {
  ...
  function nextPage(results) {
    if (results) {
      ...
      if (!argv['non-interactive'] && names.length) {
        ...
        return prompForNextPage().then(next => {
          if (next) {
            return runPage();
          }
        });
