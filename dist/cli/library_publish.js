...
const {
  CLILibraryContributeCommandSite
} = require('./library_upload');

const {
  LibraryPublishCommand,
  LibraryPublishCommandSite
} = require('../cmd');
...
module.exports.command = async (apiJS, argv) => {
  ...
  if (!site.libraryIdent()) {
    ...
    const contributeSite = new CLILibraryPublishContributeCommandSite(argv, process.cwd(), buildAPIClient(apiJS)); // todo - set more stringent validation on the contribute command since this is pre-publish
