...
describe('Cloud Command-Line Interface', () => {
  ...
  describe('Top-Level `cloud` Namespace', () => {
    ...
    it('Includes help', () => {
      ...
      commandProcessor.showHelp(helpText => {
        expect(helpText).to.equal(['Access Particle cloud functionality', 'Usage: particle cloud <command>', 'Help:  particle help cloud <command>', '', 'Commands:', '  list     Display a list of your devices, as well as their variables and functions', '  claim    Register a device with your user account with the cloud', '  remove   Release a device from your account so that another user may claim it', '  name     Give a device a name!', '  flash    Pass a binary, source file, or source directory to a device!', '  compile  Compile a source file, or directory using the cloud compiler', '  nyan     Make your device shout rainbows', '  login    Login to the cloud and store an access token locally', '  logout   Log out of your session and clear your saved access token', ''].join(os.EOL));
      });
    });
  });
  describe('`cloud list` Namespace', () => {
    ...
    it('Includes help', () => {
      ...
      commandProcessor.showHelp(helpText => {
        expect(helpText).to.equal(['Display a list of your devices, as well as their variables and functions', 'Usage: particle cloud list [options] [filter]', '', 'Param filter can be: online, offline, a platform name (photon, electron, etc), a device ID or name', ''].join(os.EOL));
      });
    });
  });
  describe('`cloud claim` Namespace', () => {
    ...
    it('Includes help', () => {
      ...
      commandProcessor.showHelp(helpText => {
        expect(helpText).to.equal(['Register a device with your user account with the cloud', 'Usage: particle cloud claim [options] <deviceID>', '', 'Examples:', '  particle cloud claim 123456789  Claim device by id to your account', ''].join(os.EOL));
      });
    });
  });
  describe('`cloud remove` Namespace', () => {
    ...
    it('Includes help', () => {
      ...
      commandProcessor.showHelp(helpText => {
        expect(helpText).to.equal(['Release a device from your account so that another user may claim it', 'Usage: particle cloud remove [options] <device>', '', 'Options:', '  --yes  Answer yes to all questions  [boolean]', '', 'Examples:', '  particle cloud remove 0123456789ABCDEFGHI  Remove device by id from your account', ''].join(os.EOL));
      });
    });
  });
  describe('`cloud name` Namespace', () => {
    ...
    it('Includes help', () => {
      ...
      commandProcessor.showHelp(helpText => {
        expect(helpText).to.equal(['Give a device a name!', 'Usage: particle cloud name [options] <device> <name>', '', 'Examples:', '  particle cloud name red green  Rename device `red` to `green`', ''].join(os.EOL));
      });
    });
  });
  describe('`cloud flash` Namespace', () => {
    ...
    it('Includes help', () => {
      ...
      commandProcessor.showHelp(helpText => {
        expect(helpText).to.equal(['Pass a binary, source file, or source directory to a device!', 'Usage: particle cloud flash [options] <device> [files...]', '', 'Options:', '  --target          The firmware version to compile against. Defaults to latest version, or version on device for cellular.  [string]', '  --followSymlinks  Follow symlinks when collecting files  [boolean]', '  --product         Target a device within the given Product ID or Slug  [string]', '', 'Examples:', '  particle cloud flash blue                                      Compile the source code in the current directory in the cloud and flash to device `blue`', '  particle cloud flash green tinker                              Flash the default `tinker` app to device `green`', '  particle cloud flash red blink.ino                             Compile `blink.ino` in the cloud and flash to device `red`', '  particle cloud flash orange firmware.bin                       Flash a pre-compiled `firmware.bin` binary to device `orange`', '  particle cloud flash 0123456789abcdef01234567 --product 12345  Compile the source code in the current directory in the cloud and flash to device `0123456789abcdef01234567` within product `12345`', ''].join(os.EOL));
      });
    });
  });
  describe('`cloud compile` Namespace', () => {
    ...
    it('Includes help', () => {
      ...
      commandProcessor.showHelp(helpText => {
        expect(helpText).to.equal(['Compile a source file, or directory using the cloud compiler', 'Usage: particle cloud compile [options] <deviceType> [files...]', '', 'Options:', '  --target          The firmware version to compile against. Defaults to latest version, or version on device for cellular.  [string]', '  --followSymlinks  Follow symlinks when collecting files  [boolean]', '  --saveTo          Filename for the compiled binary  [string]', '', 'Examples:', '  particle cloud compile photon                                  Compile the source code in the current directory in the cloud for a `photon`', '  particle cloud compile electron project --saveTo electron.bin  Compile the source code in the project directory in the cloud for an `electron` and save it to a file named `electron.bin`', '', 'Param deviceType can be: core, photon, p1, electron, argon, asom, boron, bsom, xenon, xsom, etc', ''].join(os.EOL));
      });
    });
  });
  describe('`cloud nyan` Namespace', () => {
    ...
    it('Includes help', () => {
      ...
      commandProcessor.showHelp(helpText => {
        expect(helpText).to.equal(['Make your device shout rainbows', 'Usage: particle cloud nyan [options] <device> [onOff]', '', 'Options:', '  --product  Target a device within the given Product ID or Slug  [string]', '', 'Examples:', '  particle cloud nyan blue                  Make the device named `blue` start signaling', '  particle cloud nyan blue off              Make the device named `blue` stop signaling', '  particle cloud nyan blue --product 12345  Make the device named `blue` within product `12345` start signaling', ''].join(os.EOL));
      });
    });
  });
  describe('`cloud login` Namespace', () => {
    ...
    it('Includes help', () => {
      ...
      commandProcessor.showHelp(helpText => {
        expect(helpText).to.equal(['Login to the cloud and store an access token locally', 'Usage: particle cloud login [options]', '', 'Options:', '  -u, --username  your username  [string]', '  -p, --password  your password  [string]', '  -t, --token     an existing Particle access token to use  [string]', '  --otp           the login code if two-step authentication is enabled  [string]', '', 'Examples:', '  particle cloud login                                              prompt for credentials and log in', '  particle cloud login --username user@example.com --password test  log in with credentials provided on the command line', '  particle cloud login --token <my-api-token>                       log in with an access token provided on the command line', ''].join(os.EOL));
      });
    });
  });
  describe('`cloud logout` Namespace', () => {
    ...
    it('Includes help', () => {
      ...
      commandProcessor.showHelp(helpText => {
        expect(helpText).to.equal(['Log out of your session and clear your saved access token', 'Usage: particle cloud logout [options]', ''].join(os.EOL));
      });
    });
  });
});