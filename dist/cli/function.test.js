...
describe('Function Command-Line Interface', () => {
  ...
  describe('Top-Level `function` Namespace', () => {
    ...
    it('Includes help with examples', () => {
      ...
      commandProcessor.showHelp(helpText => {
        expect(helpText).to.equal(['Call functions on your device', 'Usage: particle function <command>', 'Help:  particle help function <command>', '', 'Commands:', '  list  Show functions provided by your device(s)', '  call  Call a particular function on a device', ''].join(os.EOL));
      });
    });
  });
  describe('Handles `function list` Namespace', () => {
    ...
    it('Includes help', () => {
      ...
      commandProcessor.showHelp(helpText => {
        expect(helpText).to.equal(['Show functions provided by your device(s)', 'Usage: particle function list [options]', ''].join(os.EOL));
      });
    });
  });
  describe('Handles `function call` Command', () => {
    ...
    it('Includes help with examples', () => {
      ...
      commandProcessor.showHelp(helpText => {
        expect(helpText).to.equal(['Call a particular function on a device', 'Usage: particle function call [options] <device> <function> [argument]', '', 'Options:', '  --product  Target a device within the given Product ID or Slug  [string]', '', 'Examples:', '  particle function call coffee brew                                    Call the `brew` function on the `coffee` device', '  particle function call board digitalWrite D7=HIGH                     Call the `digitalWrite` function with argument `D7=HIGH` on the `board` device', '  particle function call 0123456789abcdef01234567 brew --product 12345  Call the `brew` function on the device with id `0123456789abcdef01234567` within product `12345`', ''].join(os.EOL));
      });
    });
  });
});