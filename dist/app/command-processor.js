...
const {
  JSONErrorResult
} = require('../lib/json-result'); // It's important to run yargs in the directory of the script so it picks up options from package.json
...
class CLICommandItem {
  ...
  get path() {
    return this.parent ? this.parent.path.concat([this.name]) : [this.name];
  }
  /**
  ...
  configure(yargs, {
    ...
    if (options) {
      ...
      this.configureOptions(options); // avoid converting positional arguments to numbers by default
  ...
  configureOptions(options) {
    Object.keys(options).forEach(key => {
      ...
      if (!option.hasOwnProperty('nargs') && !option.boolean && !option.count && !option.array) {
        option.nargs = 1;
      }

      if (!option.hasOwnProperty('number') && !option.hasOwnProperty('boolean') && !option.hasOwnProperty('string') && !option.hasOwnProperty('count') && !option.hasOwnProperty('array')) {
        option.string = true;
      }
    });
  }
...
class CLICommandCategory extends CLICommandItem {
  ...
  configureParser(args, yargs) {
    this.configure(yargs); // add the subcommands of this category
    ...
    yargs.usage((this.description ? this.description + '\n' : '') + ['Usage: $0', ...this.path, '<command>'].join(' ') + '\n' + ['Help:  $0 help', ...this.path, '<command>'].join(' ')).check(argv => this.check(yargs, argv));
    return yargs;
  }
...
class CLICommand extends CLICommandItem {
  ...
  configureParser(args, yargs) {
    ...
    yargs.check(argv => {
      ...
      return true;
    }).usage((this.description ? this.description + '\n' : '') + 'Usage: $0 ' + this.path.join(' ') + ' [options]' + (this.options.params ? ' ' + this.options.params : ''));
    return yargs;
  }
...
function consoleErrorLogger(console, yargs, exit, error) {
  ...
  if (error) {
    if (error.asJSON) {
      console.log(new JSONErrorResult(error).toString());
    } else {
...
function checkForUnknownArguments(yargs, argv, command) {
  ...
  function isUnknown(key) {
    return key !== '$0' && key !== '_' && key !== 'params' && !demanded.hasOwnProperty(key) && !flags.hasOwnProperty(key) && !aliasLookup.hasOwnProperty('no-' + key) && !aliasLookup.hasOwnProperty(key);
  }
...
function parseParams(yargs, argv, path, params) {
  ...
  params.replace(/(<[^>]+>|\[[^\]]+\])/g, match => {
    ...
    const param = match.slice(1, -1).replace(/(.*)\.\.\.$/, (m, param) => {
      variadic = true;
      return param;
    });
...
function baseError(message, data) {
  const error = new Error(); // yargs doesn't pass the full error if a message is defined, only the message
