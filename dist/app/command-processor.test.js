...
describe('command-line parsing', () => {
  ...
  it('returns the category when the alias matches', () => {
    const app = commandProcessor.createAppCategory();
    const one = commandProcessor.createCategory(app, 'one', {
      alias: 'uno'
    });
  ...
  it('refuses options meant for other commands', () => {
    ...
    expect(argv).to.have.property('one').equal(true); // the real test
  ...
  describe('parameters', () => {
    function paramsCommand(params, args) {
      ...
      const result = commandProcessor.parse(app, ['cmd'].concat(args)); // only one of them set
  ...
  describe('consoleErrorLogger()', () => {
    ...
    it('logs the stack trace to the console when verbose mode is enabled', () => {
      ...
      try {
        ...
        consoleErrorLogger(fakeConsole, undefined
        /*yargs*/
        , false, error);
      } finally {
