...
module.exports = class WebhookCommand {
  ...
  _createHook({
    ...
    api.ensureToken(); //if they gave us one thing, and it happens to be a file, and we could parse it as json

    let data = {}; //particle webhook create xxx.json
    ...
    if (eventName && !url && !deviceID) {
      ...
      if (getFilenameExt(filename).toLowerCase() === '.json') {
        ...
        console.log('Using settings from the file ' + filename); //only override these when we didn't get them from the command line
  ...
  listHooks() {
    ...
    return api.listWebhooks().then(hooks => {
      ...
      for (let i = 0; i < hooks.length; i++) {
        const hook = hooks[i];
        const line = ['    ', i + 1, '.) Hook ID ' + hook.id + ' is watching for ', '"' + hook.event + '"', '\n       ', ' and sending to: ' + hook.url, hook.deviceID ? '\n       ' + ' for device ' + hook.deviceID : '', '\n       ', ' created at ' + hook.created_at, '\n'].join('');
        console.log(line);
      }
    }).catch(err => {
