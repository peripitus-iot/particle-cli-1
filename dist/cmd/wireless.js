...
function updateWarning() {
  protip('Your Photon may start a', chalk.cyan('firmware update'), 'immediately upon connecting for the first time.');
  protip(chalk.white('If it starts an update, you will see it flash'), chalk.magenta('MAGENTA'), chalk.white('until the update has completed.'));
}
...
module.exports = class WirelessCommand {
  ...
  list(macAddress, manual) {
    ...
    if (manual) {
      ...
      protip('We will', chalk.cyan('automagically'), 'change the', chalk.cyan('Wi-Fi'), 'network to which your computer is connected.');
  ...
  networks(err, dat) {
    ...
    if (err) {
      protip('Some computers may require', chalk.cyan('Administrator'), 'permissions for my', chalk.cyan('automagical'), 'Wi-Fi scanning capabilities.');
      console.log();
    ...
    if (detectedDevices.length > 1) {
      ...
      this.prompt([{
        type: 'confirm',
        name: 'setupSingle',
        message: util.format('Found "%s". Would you like to perform setup on this one now?', chalk.bold.cyan(detectedDevices[0])),
        default: true
      }]).then(singleChoice);
    } else {
      console.log(arrow, chalk.bold.white('No nearby Photons detected.'), chalk.bold.white('Try the', '`' + chalk.bold.cyan(cmd + ' help') + '` command for more information.')); // Monitor for new Photons?
    ...
    function multipleChoice(ans) {
      if (ans.setup) {
        self.batch = false; // Select any/all Photons to setup
  ...
  setup(photon, cb) {
    ...
    if (mgr.supported.getCurrentNetwork) {
      mgr.getCurrentNetwork((err, network) => {
        ...
        if (network.indexOf('Photon-') === 0) {
          console.log(chalk.bold.red('!'), chalk.bold.white('You are still connected to your Photon\'s Wi-Fi network. Please reconnect to a Wi-Fi network with internet access.'));
          console.log();
          self.prompt([{
            type: 'confirm',
    ...
    function afterClaim(err, dat) {
      ...
      console.log();
      self.claimCode = dat.claim_code; // todo - prompt for manual connection before getting the claim code since this exits the setup process
    ...
    function connected(err, opts) {
      ...
      if (err) {
        ...
        console.log(chalk.bold.red('!'), chalk.bold.white('Woops. Something went wrong connecting to ' + photon + '. Please manually re-connect to your Wi-Fi network.'));
        return;
      }

      console.log(arrow, 'Hey! We successfully connected to', chalk.bold.cyan(opts.ssid));
  ...
  configure(ssid, cb) {
    console.log(); // todo - distinguish Photon/P1
    ...
    function scanChoice(ans) {
      if (!ans.auto) {
        return self.prompt([{
          ...
          choices: ['None', 'WEP Shared', 'WPA TKIP', 'WPA AES', 'WPA2 AES', 'WPA2 TKIP', 'WPA2 Mixed', 'WPA2', 'WPA2 Enterprise', 'WPA2 Enterprise AES', 'WPA2 Enterprise TKIP', 'WPA2 Enterprise Mixed', 'WPA Enterprise AES', 'WPA Enterprise TKIP']
        }, {
          type: 'input',
          name: 'password',
    ...
    function start() {
      ...
      if (retries >= 9) {
        // scan has failed 10 times already
        self.stopSpin();
        console.log(arrow, 'Your Photon failed to scan for nearby Wi-Fi networks.');
        retries = 0;
        self.prompt([{
          type: 'confirm',
          name: 'manual',
    ...
    function results(err, dat) {
      ...
      if (err) {
        console.error(err);
        console.log(arrow, 'Your Photon encountered an error while trying to scan for nearby Wi-Fi networks. Retrying...');
    ...
    function networkChoices(ans) {
      ...
      console.log(arrow, 'Security:', chalk.bold.cyan(visibleSecurity));
    ...
    function info() {
      clearTimeout(retry);
      console.log();
      console.log(arrow, 'Obtaining device information...'); // todo - this is the first attempt to connect to the photon
    ...
    function done(err) {
      ...
      self.stopSpin();
      clearTimeout(retry);
      self.stopSpin(); //console.log(arrow, chalk.bold.white('Configuration complete! You\'ve just won the internet!'));
    ...
    function checkDevices(err, dat) {
      ...
      self.prompt([{
        ...
        choices: [{
          ...
          name: 'Reconfigure the Wi-Fi settings of the Photon',
          value: 'reconfigure'
        }]
      }]).then(recheck);
  ...
  exit() {
    console.log();
    console.log(arrow, chalk.bold.white('Ok, bye! Don\'t forget `' + chalk.bold.cyan(cmd + ' help') + '` if you\'re stuck!', chalk.bold.magenta('<3')));
    process.exit(0);
  }
