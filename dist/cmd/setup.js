...
const {
  banner,
  retryDeferred
} = require('../lib/utilities'); // this is mainly so we only break 80 columns in one place.
...
module.exports = class SetupCommand {
  ...
  setup({
    ...
    function promptSwitch() {
      console.log(arrow, util.format(strings.alreadyLoggedIn, chalk.bold.cyan(settings.username)));
      self.prompt([{
        type: 'confirm',
        name: 'switch',
  ...
  signup(cb, tries) {
    if (!tries) {
      ...
      return console.log(alert, util.format(strings.helpForMoreInfo, chalk.bold.cyan(cmd)));
    }
    ...
    function signupInput(ans) {
      if (ans.confirm !== ans.password) {
        ...
        console.log(arrow, "Sorry, those passwords didn't match. Let's try again!");
  ...
  findDevice() {
    ...
    console.log();
    console.log(chalk.cyan('!'), 'PROTIP:', chalk.white('Hold the'), chalk.cyan('MODE/SETUP'), chalk.white('button on your device until it'), chalk.cyan('blinks blue!'));
    console.log(chalk.cyan('!'), 'PROTIP:', chalk.white('Please make sure you are'), chalk.cyan('connected'), chalk.white('to the'), chalk.cyan('internet.'), '\n');
    ...
    return serial.findDevices().then(devices => {
      ...
      if (devices.length > 0) {
        if (devices.length > 1) {
          console.log(alert, 'NOTICE:', chalk.blue('Multiple devices detected. Please run', chalk.bold.cyan(cmd + ' setup'), 'again to setup subsequent devices.'));
          ...
          if (ans.setup) {
            ...
            serial.supportsClaimCode(device).then(supported => {
              if (supported && !self.forceWiFi) {
                self.stopSpin();
                console.log(chalk.cyan('!'), 'The device supports setup via serial.');
              ...
              serial.getDeviceMacAddress(device).then(mac => {
                ...
                self.stopSpin();
                console.log(chalk.cyan('!'), "The Photon supports secure Wi-Fi setup. We'll try that.");
    ...
    function detectedPrompt(name, cb) {
      console.log(arrow, 'I have detected a', chalk.cyan(name), 'connected via USB.');
      self.prompt([{
        type: 'confirm',
        name: 'setup',
  ...
  setupCore(device) {
    ...
    function promptForListen() {
      ...
      return self.prompt([question]).then(answer => {
        if (answer.listen) {
    ...
    Promise.resolve().then(() => {
      ...
      return self.prompt([question]).then(ans => {
        deviceName = ans.coreName;
        return self.api.signalDevice(deviceId, false).then(() => self.api.renameDevice(deviceId, deviceName));
      });
    }).then(() => {
      console.log();
