...
module.exports = class ProductCommand extends CLICommandBase {
  ...
  addDevice({
    ...
    return this.ui.showBusySpinnerUntilResolved(msg, upload).then(result => this.showDeviceAddResult(result));
  }
  ...
  addDevices({
    ...
    if (!deviceID && !file) {
      throw usageError('`deviceID` parameter or `--file` option is required');
    }
  ...
  showDeviceAddResult({
    ...
    if (identifiers.length) {
      message.push('Success!', '', `Product ${product} Includes:`, dedupeAndStringifyIDList(identifiers), '');
    ...
    if (nonmemberDeviceIds.length) {
      message.push('Skipped Non-Member IDs:', dedupeAndStringifyIDList(nonmemberDeviceIds), '');
    }

    if (invalidDeviceIds.length) {
      message.push('Skipped Invalid IDs:', dedupeAndStringifyIDList(invalidDeviceIds), '');
    }
  ...
  removeDevice({
    ...
    const remove = createAPI().removeDevice(deviceID, product).catch(error => {
    ...
    return this.ui.showBusySpinnerUntilResolved(msg, remove).then(() => this.showDeviceRemoveResult({
      product,
      deviceID
    }));
  }

  showDeviceRemoveResult({
    product,
    deviceID
  }) {
  ...
  showDeviceDetail({
    ...
    return (json ? fetchData : this.ui.showBusySpinnerUntilResolved(msg, fetchData)).then(res => {
      if (json) {
        this.ui.stdout.write(createJSONResult(null, res));
      } else {
  ...
  showDeviceList({
    ...
    const fetchData = createAPI().listDevices({
      product,
      page,
      groups,
      perPage: limit,
    ...
    return (json ? fetchData : this.ui.showBusySpinnerUntilResolved(msg, fetchData)).then(res => {
      if (json) {
        this.ui.stdout.write(createJSONResult(page, res.devices));
      } else {
...
function uploadProductDevices(product, identifiers) {
  ...
  return createAPI().addDeviceToProduct(null, product, file).then(status => ({
    product,
    identifiers,
    status
  })).catch(error => {
...
function hasDeviceIdentifier(x = '', ids = []) {
  ...
  return ids.map(id => id.toLowerCase().trim()).some(id => id === x);
}
