...
const ensureError = require('../lib/utilities').ensureError; // TODO: DRY this up somehow
...
module.exports = class SerialCommand {
  ...
  _promptForListeningMode() {
    console.log(chalk.cyan('!'), 'PROTIP:', chalk.white('Hold the'), chalk.cyan('SETUP'), chalk.white('button on your device until it'), chalk.cyan('blinks blue!'));
    return prompt([{
      type: 'input',
      name: 'listening',
  ...
  flashDevice(binary, {
    ...
    return Promise.resolve().then(() => {
    ...
    }).then(() => this.whatSerialPortDidYouMean(port, true)).then(_device => {
      device = _device;
      ...
      const ymodem = new YModem(serialPort, {
        debug: true
      });
  ...
  _getWifiInformation(device, networks) {
    ...
    return prompt(questions).then(answers => {
      ...
      return this.serialWifiConfig(device, {
        network,
        security
      });
    });
  ...
  setup(device) {
    ...
    function afterClaim(err, dat) {
      ...
      return self.sendClaimCode(device, dat.claim_code).then(() => {
        console.log('Claim code set. Now setting up Wi-Fi'); // todo - add additional commands over USB to have the device scan for Wi-Fi
    ...
    function getClaim() {
      ...
      api.getClaimCode().then(response => {
        console.log("response from device claims");
  ...
  sendClaimCode(device, claimCode) {
    ...
    return this.doSerialInteraction(device, 'C', [[expectedPrompt, 2000, (deferred, next) => {
      next(claimCode + '\n');
  ...
  doSerialInteraction(device, command, interactions) {
    ...
    const promise = new Promise((resolve, reject) => {
      ...
      const parser = new SerialBatchParser({
        timeout: 250
      });
  ...
  serialWifiConfig(...args) {
    return this._serialWifiConfig(...args).then(() => {
  ...
  _serialWifiConfig(device, opts = {}) {
    ...
    const promise = new Promise((resolve, reject) => {
      ...
      serialTrigger.addTrigger('Password:', cb => {
        resetTimeout(); // Skip password prompt as appropriate
      ...
      serialPort.open(err => {
        ...
        serialPort.write('w');
        serialPort.drain(); // In case device is not in listening mode.
  ...
  getDeviceMacAddress(device) {
    ...
    return this._issueSerialCommand(device, 'm').then(data => {
      ...
      if (matches) {
        let mac = matches[0].toLowerCase(); // manufacturing firmware can sometimes not report the full MAC
        // lets try and fix it
        ...
        if (mac.length < 17) {
          ...
          const usiMacs = [['6c', '0b', '84'], ['44', '39', 'c4']];
          usiMacs.some(usimac => {
