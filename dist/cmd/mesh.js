...
module.exports = class MeshCommand {
  constructor(settings) {
    ...
    this._api = new ParticleApi(settings.apiUrl, {
  ...
  create(args) {
    ...
    return this._getDevice(args.params.device).then(d => {
      ...
      if (!args.yes) {
        ...
        p = p.then(() => prompt(question)).then(r => {
          if (!r.remove) {
      ...
      return prompt(questions).then(r => {
      ...
      const p = this._api.createMeshNetwork(spec).then(r => r.body.network.id);
  ...
  add(args) {
    ...
    let networkId = null; // Get the assisting device
    ...
    return this._getDevice(args.params.assisting_device).then(d => {
      ...
      networkId = assistDevice.network.id; // Open the assisting device

      return this._openUsbDeviceById(assistDevice.id, args.params.assisting_device);
    }).then(d => {
      assistUsbDevice = d; // Get the joiner device. Do not fail if the device is not claimed
      ...
      joinerUsbDevice = d; // Check if the joiner device is already a member of some network
      ...
      return p.then(() => {
        ...
        return prompt(question).then(r => r.password);
      }).then(password => {
        ...
        if (!joinerDevice) {
          ...
          p = p.then(() => this._api.removeMeshNetworkDevice({
            deviceId: joinerUsbDevice.id,
            auth: this._auth
          }));
        } // Register the joiner device with the cloud


        p = p.then(() => this._api.addMeshNetworkDevice({
          networkId,
          deviceId: joinerUsbDevice.id,
          auth: this._auth
        }));
        ...
        if (!joinerDevice) {
          const p = this._api.getClaimCode({
            auth: this._auth
          }).then(r => {
  ...
  remove(args) {
    ...
    return this._getDevice(args.params.device).then(d => {
      ...
      return p.then(() => this._openUsbDeviceById(device.id, args.params.device)).then(d => {
        usbDevice = d;
  ...
  list(args) {
    return Promise.resolve().then(() => {
      ...
      const p = this._api.listMeshNetworks({
        auth: this._auth
      }).then(r => {
      ...
      if (listDevices) {
        p = p.then(() => asyncMapSeries(networks, network => {
          ...
          return this._api.listMeshNetworkDevices(spec).then(r => {
  ...
  info(args) {
    let usbDevice = null; // Open the device

    return this._openUsbDeviceById(args.params.device).then(d => {
      usbDevice = d; // Get the network info
  ...
  _getNetwork(networkId) {
    const p = Promise.resolve().then(() => this._api.getMeshNetwork({
      networkId,
      auth: this._auth
    })).then(r => {
      return r.body;
