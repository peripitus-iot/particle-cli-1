...
module.exports = class VariableCommand extends CLICommandBase {
  ...
  listVariables() {
    return this.getAllVariablesWithCache().then(devices => this.ui.logDeviceDetail(devices, {
      varsOnly: true
    })).catch(err => {
  ...
  getValue({
    ...
    if (product) {
      if (!device) {
        return this.showUsageError('`device` parameter is required when `--product` flag is set');
      } else if (!this.isDeviceId(device)) {
        return this.showProductDeviceNameUsageError(device);
      }

      if (!variableName) {
        return this.showUsageError(`\`variableName\` parameter is required when \`--product\` flag is set. To view available variables, run: particle product device list ${product}`);
      }
      ...
      return this.ui.showBusySpinnerUntilResolved(msg, fetchVar).then(res => {
    ...
    return Promise.resolve().then(() => {
      if (!device && !variableName) {
        ...
        return this.disambiguateGetValue({
          device
        }).then(({
          deviceIds,
          variableName
      ...
      return this._getValue(device, variableName, {
        time
      });
    }).catch(err => {
  ...
  _getValue(deviceId, variableName, {
    ...
    return Promise.all(getVariables).then(results => {
  ...
  monitorVariables({
    ...
    return Promise.resolve().then(() => {
      ...
      return this._pollForVariable(deviceIds, variableName, {
        delay,
        time
      });
    }).catch(err => {
  ...
  _pollForVariable(deviceIds, variableName, {
    delay,
    time
  }) {
    const retry = () => setTimeout(this._pollForVariable.bind(this, deviceIds, variableName, {
      delay,
      time
    }), delay);
    ...
    return this._getValue(deviceIds, variableName, {
      time
    }).then(retry).catch(retry);
  }

  disambiguateGetValue({
    device,
    variableName
  }) {
    ...
    return this.getAllVariablesWithCache().then(deviceList => {
      if (device) {
        ...
        return prompt([{
          ...
          return {
            deviceIds: [device],
            variableName: answers.variableName
          };
        });
      }
