...
module.exports = class ParticleApi {
  ...
  removeAccessToken(username, password, token) {
    return this.api.removeAccessToken({
      username,
      password,
      token
    }).then(() => {
  ...
  getUserInfo() {
    return this._wrap(this.api.getUserInfo({
      auth: this.accessToken
    }));
  }

  listDevices(options) {
    return this._wrap(this.api.listDevices(Object.assign({
      auth: this.accessToken
    }, options)));
  }
  ...
  addDeviceToProduct(deviceId, product, file) {
    return this._wrap(this.api.addDeviceToProduct({
      product,
      deviceId,
      file,
      auth: this.accessToken
  ...
  markAsDevelopmentDevice(deviceId, development, product) {
    return this._wrap(this.api.markAsDevelopmentDevice({
      development,
      deviceId,
      product,
      auth: this.accessToken
  ...
  claimDevice(deviceId, requestTransfer) {
    return this._wrap(this.api.claimDevice({
  ...
  removeDevice(deviceId, product) {
    return this._wrap(this.api.removeDevice({
      product,
      deviceId,
      auth: this.accessToken
    }));
  ...
  flashDevice(deviceId, files, targetVersion, product) {
    return this._wrap(this.api.flashDevice({
      deviceId,
      product,
  ...
  signalDevice(deviceId, signal, product) {
    return this._wrap(this.api.signalDevice({
      deviceId,
      product,
      signal,
      auth: this.accessToken
  ...
  listBuildTargets(onlyFeatured) {
    return this._wrap(this.api.listBuildTargets({
      onlyFeatured,
      auth: this.accessToken
    }));
  }
  ...
  compileCode(files, platformId, targetVersion) {
    return this._wrap(this.api.compileCode({
      platformId,
      targetVersion,
      // TODO (mirande): callers should provide an object like: { [filename]: filepath }
      files: files.map || files,
  ...
  getVariable(deviceId, name, product) {
    return this._wrap(this.api.getVariable({
      name,
      deviceId,
      product,
      auth: this.accessToken
  ...
  callFunction(deviceId, name, argument, product) {
    return this._wrap(this.api.callFunction({
      name,
      argument,
      deviceId,
      product,
  ...
  downloadFirmwareBinary(binaryId) {
    return this._wrap(this.api.downloadFirmwareBinary({
      binaryId,
      auth: this.accessToken
    }));
  }
  ...
  getEventStream(deviceId, name, product) {
    return this._wrap(this.api.getEventStream({
      name,
      deviceId,
      product,
      auth: this.accessToken
  ...
  publishEvent(name, data, isPrivate, product) {
    return this._wrap(this.api.publishEvent({
      name,
      data,
      product,
      isPrivate,
  ...
  _wrap(promise) {
    return Promise.resolve(promise).then(result => result.body || result).catch(this._checkToken);
  }
  ...
  _debug(req) {
    if (global.verboseLevel > 3) {
      ...
      let destUrl; // superagent vs plain http
