...
module.exports = class FlashCommand {
  ...
  flashYModem({
    ...
    return new SerialCommands().flashDevice(binary, {
      port,
      yes
    });
  }
  ...
  flashDfu({
    ...
    return Promise.resolve().then(() => dfu.isDfuUtilInstalled()).then(() => dfu.findCompatibleDFU()).then(() => {
      ...
      const alt = 0; // todo - leave on factory firmware write too?
  ...
  _dropModuleInfo(binary) {
    ...
    return new Promise((resolve, reject) => {
      const rStream = fs.createReadStream(binary, {
        start: ModuleInfo.HEADER_SIZE
      });
      const wStream = temp.createWriteStream({
        suffix: '.bin'
      });
      rStream.pipe(wStream).on('error', reject).on('finish', () => resolve(wStream.path));
    });
  }
