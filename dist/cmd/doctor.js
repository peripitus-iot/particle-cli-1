...
module.exports = class DoctorCommand {
  ...
  deviceDoctor() {
    ...
    return Promise.resolve().then(this._setupApi.bind(this)).then(this._findDevice.bind(this)).then(this._nameDevice.bind(this)).then(this._updateSystemFirmware.bind(this)).then(this._updateCC3000.bind(this)).then(this._flashDoctor.bind(this)).then(this._selectAntenna.bind(this)).then(this._selectIP.bind(this)).then(this._resetSoftAPPrefix.bind(this)).then(this._clearEEPROM.bind(this)).then(this._setupWiFi.bind(this)).then(this._resetKeys.bind(this)).then(this._flashTinker.bind(this)).then(this._showDoctorGoodbye.bind(this)).catch(this._showDoctorError.bind(this));
  }
  ...
  _showDoctorWelcome() {
    ...
    _.map(['Upgrade system firmware', 'Flash the default Tinker app', 'Reset the device and server keys', 'Clear the Wi-Fi settings'], line => {
      console.log('  - ' + line);
    });
  }
  ...
  _enterDfuMode() {
    ...
    console.log('Tap ' + chalk.bold.cyan('RESET/RST') + ' while holding ' + chalk.bold.cyan('MODE/SETUP') + ' until the device blinks ' + chalk.bold.yellow('yellow.'));
  ...
  _updateSystemFirmware() {
    ...
    return this._enterDfuMode().then(() => {
  ...
  _updateCC3000() {
    ...
    return this._enterDfuMode().then(() => {
      return this.command('flash').flashDfu({
        binary: 'cc3000'
      });
    }).then(() => {
  ...
  _resetKeys() {
    this._displayStepTitle('Resetting server and device keys'); // do this again to refresh the device data with latest firmware


    return this._waitForSerialDevice(deviceTimeout).then(() => this._verifyDeviceOwnership()).then(() => this._enterDfuMode()).then(() => {
  ...
  _verifyDeviceOwnership() {
    return Promise.resolve().then(() => {
      ...
      return this.api.getAttributes(this.device.deviceId).then(attributes => attributes.error !== 'Permission Denied').catch(() => false);
    }).then(ownsDevice => {
      if (ownsDevice) {
        return;
      }
