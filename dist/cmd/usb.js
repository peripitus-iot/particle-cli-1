...
module.exports = class UsbCommand {
  ...
  list(args) {
    ...
    const filterFunc = buildDeviceFilter(filter); // Enumerate USB devices
  ...
  setSetupDone(args) {
    ...
    return this._forEachUsbDevice(args, usbDevice => {
      if (usbDevice.isMeshDevice) {
        return usbDevice.setSetupDone(done).then(() => {
          if (done) {
            return usbDevice.leaveListeningMode();
          }
  ...
  configure() {
    ...
    return installUdevRules().then(() => console.log('Done.'));
  }
  ...
  cloudStatus(args, started = Date.now()) {
    ...
    const deviceMgr = {
      ...
      status() {
        ...
        let getStatus = usbDevice ? usbDevice.getCloudConnectionStatus() : Promise.resolve('unknown');
    ...
    const queryDevice = openUsbDeviceById(options).then(usbDevice => deviceMgr.set(usbDevice).status());
    return spin(queryDevice, 'Querying device...').then(status => {
      ...
      if (until) {
        return deviceMgr.close().then(() => this.cloudStatus(args, started));
      }
  ...
  _forEachUsbDevice(args, func, {
    dfuMode = false
  } = {}) {
    const msg = 'Getting device information...';

    const operation = this._openUsbDevices(args, {
      dfuMode
    });

    let lastError = null;
    return spin(operation, msg).then(usbDevices => {
      const p = usbDevices.map(usbDevice => {
        return Promise.resolve().then(() => func(usbDevice)).catch(e => lastError = e).finally(() => usbDevice.close());
      });
