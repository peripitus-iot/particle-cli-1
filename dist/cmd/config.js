...
module.exports = class ConfigCommand {
  ...
  configSwitch(profile, setting, value, {
    list
  }) {
    if (list) {
      this.listProfiles();
  ...
  listProfiles() {
    ...
    const files = utilities.globList(null, [path.join(particleDir, '*.config.json')]);
    ...
    if (files.length > 0) {
      ...
      for (let i = 0; i < files.length; i++) {
        //strip the path
        const filename = path.basename(files[i]); //strip the extension
