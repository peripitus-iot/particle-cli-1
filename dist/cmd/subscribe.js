...
module.exports = class SubscribeCommand extends CLICommandBase {
  ...
  startListening({
    ...
    if (all && !event) {
      return this.showUsageError('`event` parameter is required when `--all` flag is set');
    }
    ...
    return this.ui.showBusySpinnerUntilResolved('Fetching event stream...', fetchStream).then(stream => {
      this.ui.stdout.write(os.EOL);
      return stream;
    }).then(stream => stream.on('event', this.createEventHandler(until, max))).catch(error => {
