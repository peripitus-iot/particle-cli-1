...
function doUpdate(id) {
  ...
  if (!updates) {
    console.log();
    return console.log(chalk.cyan('!'), 'There are currently no system firmware updates available for this device.');
  }
  ...
  parts.forEach((part, partNumber) => {
    steps.push(next => {
      ...
      return parser.parseFile(binary).catch(() => {
        return;
      }).then(mod => {
        if (mod && mod.prefixInfo.moduleFunction !== ModuleInfo.FunctionType.BOOTLOADER) {
          ...
          return new FlashCommand().flashDfu({
            binary: binary,
            requestLeave: leave
          }).catch(err => {
            throw new Error(err);
      ...
      }).then(result => delay(2000).then(() => result)).then(result => next(null, result)).catch(error => next(error));
    });
  });
  console.log();
...
function dfuError(err) {
  if (err === 'No DFU device found') {// do nothing
  } else if (err.code === 127) {
    dfuInstall(true);
  } else {
    dfuInstall(false);
    console.log(chalk.cyan('!!!'), 'You may also find our community forums helpful:\n', chalk.bold.white('https://community.particle.io/'), '\n');
    console.log(chalk.red.bold('>'), 'Error code:', chalk.bold.white(err.code || 'unknown'), '\n');
  }
