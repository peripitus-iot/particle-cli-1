...
module.exports = class KeysCommand {
  ...
  showTransportProtocol() {
    return Promise.resolve().then(() => this.dfu.isDfuUtilInstalled()) //make sure our device is online and in dfu mode
    .then(() => this.dfu.findCompatibleDFU()).then(() => this.validateDeviceProtocol()).then(protocol => {
  ...
  changeTransportProtocol(protocol) {
    ...
    return Promise.resolve().then(() => this.dfu.isDfuUtilInstalled()) //make sure our device is online and in dfu mode
    .then(() => this.dfu.findCompatibleDFU()).then(() => {
  ...
  _makeNewKey({
    ...
    return Promise.resolve().then(() => {
      return Promise.resolve().then(() => this.dfu.isDfuUtilInstalled()).then(() => this.dfu.findCompatibleDFU(showHelp)).catch(err => {
        if (protocol) {
    ...
    }).then(() => this.makeKeyOpenSSL(filename, alg, {
      protocol
    })).then(() => {
  ...
  _writeKeyToDevice({
    ...
    return Promise.resolve().then(() => this.dfu.isDfuUtilInstalled()) //make sure our device is online and in DFU mode
    .then(() => this.dfu.findCompatibleDFU()).then(() => this.validateDeviceProtocol()).then(_protocol => {
      protocol = _protocol; //backup their existing key so they don't lock themselves out.
      ...
      return this._saveKeyFromDevice({
  ...
  _saveKeyFromDevice({
    ...
    return Promise.resolve().then(() => this.dfu.isDfuUtilInstalled()).then(() => this.dfu.findCompatibleDFU()).then(() => this.validateDeviceProtocol()).then(_protocol => {
      protocol = _protocol;
      ...
      return deferredChildProcess(`openssl ${alg} -in "${filename}" -inform DER -pubout -out ${pubPemFilename}`).catch(err => {
  ...
  sendPublicKeyToServer({
    ...
    return this._sendPublicKeyToServer({
      deviceID,
      filename,
      productId,
      algorithm: 'rsa'
  ...
  _sendPublicKeyToServer({
    ...
    return Promise.resolve().then(() => {
      ...
      return deferredChildProcess(`openssl ${algorithm} -inform ${inform} -in "${filename}" -pubout -outform PEM -out "${pubKey}"`).catch(() => {
  ...
  keyDoctor({
    ...
    return Promise.resolve().then(() => this.dfu.isDfuUtilInstalled()).then(() => this.dfu.findCompatibleDFU()).then(() => this.validateDeviceProtocol({
      protocol
    })).then(_protocol => {
      protocol = _protocol;
    ...
    }).then(() => this._writeKeyToDevice({
      filename,
      leave: true
    })).then(() => this._sendPublicKeyToServer({
      deviceID,
      filename,
      algorithm
    })).then(() => {
  ...
  _createAddressBuffer(ipOrDomain) {
    const isIpAddress = /^[0-9.]*$/.test(ipOrDomain); // create a version of this key that points to a particular server or domain
  ...
  writeServerPublicKey({
    ...
    if (deviceType && !filename) {
      throw usageError('`filename` parameter is required when `--deviceType` is set');
    }
    ...
    if (deviceType) {
      skipDFU = true; // Lookup the DFU ID string that matches the provided deviceType:
    ...
    return Promise.resolve().then(() => {
      ...
      return this.dfu.isDfuUtilInstalled().then(() => this.dfu.findCompatibleDFU()).then(() => this.validateDeviceProtocol({
        protocol
      }));
    }).then(_protocol => {
      protocol = _protocol;
      ...
      return this._formatPublicKey(derFile, host, port, {
        protocol,
        outputFilename
      });
    }).then(bufferFile => {
  ...
  readServerAddress({
    ...
    return Promise.resolve().then(() => this.dfu.isDfuUtilInstalled()).then(() => this.dfu.findCompatibleDFU()).then(() => this.validateDeviceProtocol({
      ...
      return this.dfu.readBuffer(segment, false).then(buf => {
        keyBuf = buf;
      });
    }).then(() => {
  ...
  validateDeviceProtocol({
    specs,
    protocol
  } = {}) {
    specs = specs || deviceSpecs[this.dfu.dfuId];
    return protocol ? Promise.resolve(protocol) : this.fetchDeviceProtocol(specs).then(detectedProtocol => {
  ...
  _getDERPublicKey(filename, {
    ...
    let variant = this._getServerKeyVariant({
      protocol
    });

    if (!filename) {
      filename = this.serverKeyFilename({
        alg,
        variant
      });
    }
    ...
    if (getFilenameExt(filename).toLowerCase() !== '.der') {
      ...
      if (!fs.existsSync(derFile)) {
        ...
        return derFilePromise.then(() => derFile).catch(err => {
  ...
  _formatPublicKey(filename, ipOrDomain, port, {
    ...
    if (ipOrDomain) {
      ...
      let addressBuf = this._createAddressBuffer(ipOrDomain); // To generate a file like this, just add a type-length-value (TLV)
      ...
      buf = new Buffer(segment.size); //copy in the key

      fileBuf = fs.readFileSync(filename);
      fileBuf.copy(buf, 0, 0, fileBuf.length); //fill the rest with "FF"
