...
module.exports = class ApiClient {
  ...
  createUser(user, pass) {
    ...
    if (!user || user === '' || !user.includes('@') || !user.includes('.')) {
  ...
  _mapFilenames(fileMapping, messages) {
    ...
    let result = [];
    let map = {}; // prepend each logical path with a slash (since the compile server does that.)
  ...
  getEventStream(eventName, deviceId, onDataHandler) {
    ...
    return new Promise((resolve, reject) => {
      ...
      }).on('error', err => reject(err)).on('cloud', data => resolve(data)).on('data', data => {
        if (failed) {
  ...
  publishEvent(eventName, data, setPrivate) {
    ...
    return new Promise((resolve, reject) => {
      ...
      request(options, (error, response, body) => {
        ...
        console.log(`Published ${setPrivate ? 'private' : 'public'}`, 'event:', eventName);
        console.log('');
        console.log(body);
        resolve(body);
      });
  ...
  hasBadToken(body) {
    if (body && body.error && body.error.indexOf && body.error.indexOf('invalid_token') >= 0) {
