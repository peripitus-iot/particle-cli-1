...
describe('Windows wifi', () => {
  ...
  describe('_connectProfile', () => {
    ...
    it('runs netsh wlan connect', () => {
      ...
      expect(connector._execWiFiCommand).to.be.calledWith(['connect', 'name=blah', 'interface=may contain spaces']);
    });
  });
  ...
  describe('_checkHasInterface', () => {
    ...
    it('raises an error when the interface has no name', () => {
      expect(() => connector._checkHasInterface({
        ssid: 'abcd'
      })).to.throw(Error, msg);
    });
