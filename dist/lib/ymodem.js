...
module.exports = class YModem {
  ...
  send(filenames) {
    ...
    return this._open(this.options.inListeningMode).then(() => {
      return asyncMapSeries(filenames, filename => {
        console.log('sending file:', filename);
        return readFile(filename).then(buffer => self._sendFile(buffer));
      });
    }).then(() => this._endTransfer()).catch(err => {
      self.errored = true;
  ...
  _open(inListeningMode) {
    ...
    const promise = new Promise((resolve, reject) => {
      self.port.open(err => {
        ...
        function cmdResponse() {
          ...
          line += data.toString();
          const lines = line.split(linebreakPtn); // if not in listening mode, we get CRC16 back
    ...
    return enforceTimeout(promise, 10000).catch(error => {
  ...
  _sendRawPacket(packet) {
    ...
    return writeToPort.then(() => enforceTimeout(response, 70000));
  }
